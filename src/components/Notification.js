...
const NotificationText = ({type, status, context}) => {
    ...
    if (type === 'signin' && status === 'success' && context.member) {
        ...
        return (
            <p>
                No has podido iniciar sesión. El enlace de inicio de sesión ha caducado. <a href={signinPortalLink} target="_parent">Haz click aquí para probar de nuevo</a>
            </p>
        );
        ...
        return (
            <p>
                ¡Estupendo! Consulta tu correo electrónico para iniciar sesión de forma segura sin contraseña.
            </p>
        );
        ...
        if (context.member) {
            return (
                <p>
                    La actualización del plan se ha cancelado cancelada.
                </p>
            );
        }
        return (
            <p>
                El pago del plan se ha cancelado cancelado.
            </p>
        );
    }
    return (
        <p>
            {status === 'success' ? 'ëxito' : 'Error'}
        </p>
    );
};
