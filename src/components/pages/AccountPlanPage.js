...
const Header = ({onBack, showConfirmation, confirmationType}) => {
    ...
    let title = isPaidMember({member}) ? 'Cambiar de plan' : 'Elegir un plan';
    if (showConfirmation) {
...
const PlanConfirmationSection = ({plan, type, onConfirm}) => {
    ...
    const planStartMessage = `${priceString}${plan.currency_symbol}/${plan.interval} – A partir del  ${planStartDate}`;
