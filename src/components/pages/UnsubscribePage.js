...
export default function UnsubscribePage() {
    ...
    if (member === null) {
        return (
            <div className='gh-portal-content gh-portal-unsubscribe with-footer'>
                <CloseButton />
                <AccountHeader />
                <h1 className="gh-portal-main-title">Fallo al cancelar la suscripción</h1>
                <div>
    ...
    if (siteNewsletters?.length === 1 && !commentsEnabled && !showPrefs) {
        return (
            <div className='gh-portal-content gh-portal-unsubscribe with-footer'>
                <CloseButton />
                <AccountHeader />
                <h1 className="gh-portal-main-title">Has anulado la suscripción con éxito</h1>
                <div>
    ...
    const HeaderNotification = () => {
        if (pageData.comments && commentsEnabled) {
            ...
            return (
                <>
                    <p className={`gh-portal-text-center gh-portal-header-message ${hideClassName}`}><strong>{member?.email}</strong> ya no recibirá correos electrónicos cuando alguien responda a tus comentarios.</p>
                </>
            );
        }
        ...
        return (
            <>
                <p className={`gh-portal-text-center gh-portal-header-message ${hideClassName}`}><strong>{member?.email}</strong> ya no recibirá el boletín de email <strong>{unsubscribedNewsletter?.name}</strong>.</p>
            </>
        );
    };
        ...
        <NewsletterManagement
            ...
            unsubscribeAll={async () => {
                ...
                onAction('showPopupNotification', {
                    ...
                    message: `Preferencias de correo electrónico actualizadas.`
                });
